...
export class AgentTaskRequest<T extends z.ZodTypeAny> {
    ...
    correlationId: string; // Correlation ID to group all tasks of the same workflow together.
    command: Command;
...
export class AgentTaskOrchestratorResponse<T extends z.ZodTypeAny> {
    ...
    constructor(stopReason: StopReason, correlationId: string, taskOutput?: z.infer<T>, subtasks?: SubTaskInfo[], branches?: BranchInfo[]) {
        this.correlationId = correlationId;
        this.stopReason = stopReason;
        this.taskOutput = taskOutput;
